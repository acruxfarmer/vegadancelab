...
Run `scripts/diagnose-ingestion-database.ps1` first in the already-unlocked Bitwarden terminal. It reads only `Vega Dev - Supabase / DATABASE_URL` for the connection attempt. TLS certificate and hostname verification remain required. It reports allowlisted error codes without raw errors or secret values. The handoff now verifies the same existing restricted URL before contacting Render, and performs no credential generation, login activation, rotation, or operator fallback.

Required stored fields: existing `Vega Dev - Supabase / DATABASE_URL` and `Vega Dev - Render Operator / RENDER_API_KEY`. The database operator item is no longer read by these scripts. If the Render field is missing, create a personal API key in Render Account Settings → API Keys and store it as a hidden custom field in that exact existing operator item. Do not put it in `.env.local` or paste it into chat. No new API key is needed if a valid one is already stored.
