...
const staff={role:'staff',userId:'local-desk',tenantId:'local',businessId:'local',participantIds:[]},member={...staff,role:'member',userId:'local-member',participantIds:['p','waiting']};
let state={...emptyState(),participants:[{id:'p',name:'Booked Test Member'},{id:'waiting',name:'Waiting Test Member'},{id:'holder',name:'Seat Holder'}],classes:[{id:'c',title:'Class Cancellation Test',instructor:'Test Instructor',location:'Test Studio',duration:60,startsAt:new Date(Date.now()+86400000).toISOString(),status:'open',capacity:2,waitlistEnabled:true,creditRequired:true}]};
...
const prior=run('reserve',{participantId:'p',classId:'c'});run('cancel',{},prior.id);
run('reserve',{participantId:'p',classId:'c'});run('reserve',{participantId:'waiting',classId:'c'});
state.classes.push({...state.classes[0],id:'blocked',title:'Historical Attendance Block'});
const attended=run('reserve',{participantId:'p',classId:'blocked'});run('attendance',{status:'present'},attended.id);run('attendance',{status:'not_recorded',reason:'Preserve recorded history'},attended.id);run('cancel',{},attended.id);
...
const server=createServer(async(req,res)=>{
 ...
 const match=req.url?.match(/^\/api\/reservations\/([^/]+)\/(cancel|promote|attendance)$/),action=req.url==='/api/classes/cancel'?'cancel-class':req.url==='/api/reservations'?'reserve':match?.[2];
 ...
 if(req.method!=='GET')return json(405,{error:'Class cancellation fixture only'});assets.emit('request',req,res);
});
server.listen(0,'127.0.0.1',()=>console.log(`Class cancellation fixture http://127.0.0.1:${server.address().port}`));
