...
export function memberBookingOption(state,c,participantId,at,passId){
 ...
 if(state.reservations.filter(r=>r.classId===c.id&&r.status==='reserved').length>=c.capacity)return {...blocked('This class is full.'),waitlistEligible:c.waitlistEnabled===true};
 if(!c.creditRequired)return {...base,eligible:true,reason:'Eligible to book. No class credit is required.'};
 const units=eligibleCredits(state,c,participantId,at,passId),unit=units[0];
