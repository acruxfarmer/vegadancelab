...
export function createApplicationApi(env,store,fetcher=fetch){
 ...
 return async(req,res)=>{
  ...
  try{
   ...
   const body=await readJson(req),routes={'/api/reservations':'reserve','/api/classes':'class','/api/participants':'participant','/api/preferences':'preferences','/api/notifications':'notification'};
   let action=routes[url.pathname],id;
   const match=url.pathname.match(/^\/api\/reservations\/([A-Za-z0-9-]{1,128})\/(cancel|attendance|promote)$/);
