...
export function createApplicationStore(pool,{receiptPublicKey=process.env.RECEIPT_PUBLIC_KEY}={}){
 ...
 return {
  recordRevocation:({userId,sessionId,outcome})=>transaction(userId,async(c,a)=>{
   ...
   await c.query("insert into vega_private.recovery_outbox(event_id,tenant_id,business_id,actor_id,event_kind,payload,payload_digest) values($1,$2,$3,$4,'security',$5,$6) on conflict(event_id) do nothing",[receipt.eventId,a.tenantId,a.businessId,userId,receipt.payload,receipt.payloadDigest]);
  ...
  command:(userId,command)=>transaction(userId,async(c,a)=>{
   ...
   await c.query('insert into vega_private.recovery_outbox(event_id,tenant_id,business_id,actor_id,request_id,previous_revision,revision,payload,payload_digest) values($1,$2,$3,$4,$5,$6,$7,$8,$9)',[receipt.eventId,a.tenantId,a.businessId,userId,requestId,receipt.previousRevision,receipt.revision,receipt.payload,receipt.payloadDigest]);
   return {...next.result,independentReceipt:{operationId:receipt.eventId,state:'pending'}};
  }),
  close:()=>pool.end()
 };
}
