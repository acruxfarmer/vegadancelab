...
export const defaultHistoryFilters=()=>({type:'all',text:''});
const searchText=value=>Array.isArray(value)?value.map(searchText).join(' '):value&&typeof value==='object'?Object.entries(value).map(([k,v])=>k+' '+searchText(v)).join(' '):String(value??'');
export function historyResults(history,filters=defaultHistoryFilters()){
 const total=history.events.length;
 if(!['all',...reservationHistoryTypes].includes(filters.type))return {events:[],total,error:'Choose an available event type.'};
 const query=String(filters.text||'').trim().replace(/\s+/g,' ').toLowerCase();
 const events=history.events.filter(x=>{
  // Only fields rendered below; never current reservation/participant values.
  const displayed=[labels[x.type],x.recordedAt,x.actor.id,x.actor.role,x.outcome,x.reason,x.before,x.after,x.creditOutcome,x.conflicts,{eventId:x.eventId,requestId:x.requestId,records:x.evidence},...(x.type==='cancellation-correction'?[x.requested]:[])];
  return (filters.type==='all'||filters.type===x.type)&&(!query||searchText(displayed).replace(/\s+/g,' ').toLowerCase().includes(query));
 });
 return {events,total,error:''};
}
function historyControls(filters,result,e){
 return `<form id="reservation-history-filters"><div class="form-grid"><label class="field">Event type<select name="type">${['all',...reservationHistoryTypes].map(t=>`<option value="${t}" ${filters.type===t?'selected':''}>${t==='all'?'All events':labels[t]}</option>`).join('')}</select></label><label class="field">Search recorded history<input type="search" name="text" maxlength="200" value="${e(filters.text)}"></label></div><div class="row"><button class="button secondary">Apply history filters</button><button type="button" class="text-button" data-reset-history-filters>Reset history filters</button></div><p role="${result.error?'alert':'status'}">${result.error?e(result.error):`Showing ${result.events.length} of ${result.total} recorded timeline entries`}</p></form>`;
}
export function reservationHistoryHTML(data,reservationId,e,filters=defaultHistoryFilters()){
 const h=reservationHistory(data,reservationId);if(!h.occurrenceId)return '';
 const r=data.reservations.find(r=>r.id===reservationId),result=historyResults(h,filters);
