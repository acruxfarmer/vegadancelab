...
export function memberPortalUI({getData,escape:e,date,time}){
 ...
 function activity(){
  const d=getData(),items=d.reservations.flatMap(r=>{
   ...
   if(r.waitlistHistory?.length)return [...r.waitlistHistory.map(h=>({...details,at:h.createdAt,label:{joined:'Waitlist joined',left:'Waitlist left',promoted:'Booking confirmed from waitlist'}[h.action]||'Waitlist updated',detail:h.action==='promoted'?(r.creditConsumption?`1 credit used from ${pass?.label||'class credits'}.`:'No class credit required.'):'No seat or credit consumed by waiting.'})),...(r.status==='cancelled'&&r.cancellation?.originalBookingStatus!=='waitlisted'?[{...details,at:r.cancellation?.originalCancelledAt,label:'Cancellation',detail:cancellationOutcome(r,pass?.label)}]:[])];
