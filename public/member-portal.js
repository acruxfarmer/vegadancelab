...
export function memberPortalUI({getData,escape:e,date,time}){
 ...
 function activity(){
  const d=getData(),items=d.reservations.flatMap(r=>{
   ...
   if(r.waitlistHistory?.length)return [...r.waitlistHistory.map(h=>({...details,at:h.createdAt,label:{joined:'Waitlist joined',left:'Waitlist left',closed:'Waitlist closed — class cancelled',promoted:'Booking confirmed from waitlist'}[h.action]||'Waitlist updated',detail:h.action==='promoted'?(r.creditConsumption?`1 credit used from ${pass?.label||'class credits'}.`:'No class credit required.'):'No seat or credit consumed by waiting.'})),...(r.status==='cancelled'&&r.cancellation?.originalBookingStatus!=='waitlisted'?[{...details,at:r.cancellation?.originalCancelledAt,label:r.classCancellation?'Class cancelled by studio':'Cancellation',detail:cancellationOutcome(r,pass?.label)}]:[])];
   return [{...details,at:r.createdAt,label:'Booking made',detail:r.creditConsumption?`1 credit used from ${pass?.label||'class credits'}.`:'No class credit consumed.'},...(r.status==='cancelled'?[{...details,at:r.cancellation?.originalCancelledAt,label:r.classCancellation?'Class cancelled by studio':r.cancellation?.classification==='early'?'Early cancellation':r.cancellation?.classification==='late'?'Late cancellation':'Cancellation',detail:cancellationOutcome(r,pass?.label)}]:[])];
 ...
 return {entitlements,activity,notices:()=>promotionNoticesHTML(getData(),e)+cancellationNoticesHTML(getData(),e)};
}
